...
async def cancel(message: types.message, state: FSMContext):
    await message.answer(
        "Вы отменили добавление нового ученика. Нажмите /help, чтобы посмотреть список доступных команд."
    )
    await state.clear()
...
async def check_data(message: types.message, state: FSMContext):
    ...
    await message.answer(
        text=f"Вы хотите добавить ученика по имени {message.text}, верно?",
...
async def give_member_id(callback: types.CallbackQuery, state: FSMContext):
    new_member_id = get.new_member_id("student")

    await callback.message.answer(
        f"Отлично! Я зарегистрировал для Вас нового ученика. Вот его id участника: {new_member_id}. При первой авторизации ему потребуется этот id."
    )
...
async def get_name_again(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer(
        f"Если хотите отменить добавление нового ученика, нажмите /cancel. Если хотите изменить имя, введите новое."
    )
